...
export default ({ moi, msg, query }) => {
  // (()=>{

  // })()

  fetchMoi(`https://api.open5e.com/classes/?name=${toCapitalize(query[1])}`)
    .then(res => {
      const data = JSON.parse(res.body);

      const Class = data.results[0];
      ...
      const handleClassTable = table => {
        ...
        if (name == 'Bard') {
          moi.createMessage(msg.channel.id, toCode(table.substring(0, 1882)));
          moi.createMessage(
            msg.channel.id,
        ...
        if (name == 'Cleric') {
          moi.createMessage(msg.channel.id, toCode(table.substring(0, 1935)));
        ...
        if (name == 'Druid') {
          moi.createMessage(msg.channel.id, toCode(table.substring(0, 1232)));
          moi.createMessage(
            msg.channel.id,
        ...
        if (name == 'Paladin') {
          moi.createMessage(msg.channel.id, toCode(table.substring(0, 1272)));
          moi.createMessage(
            msg.channel.id,
